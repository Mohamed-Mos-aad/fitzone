        ...
        <section className="flex flex-col gap-4 items-center bg-[#0D1014] border border-[#1A1F26] px-6 sm:px-12 py-16 sm:py-24">
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                {currentPlans.map((plan) => (
                    plan.featured ? (
                        <div key={plan.id} className="w-full flex flex-col gap-8 bg-[#000000] border border-[#CCFF00] drop-shadow-2xl drop-shadow-[#CCFF00]/25 p-8 rounded-lg">
                            ...
                            <ul className="flex flex-col gap-3.5">
                                {plan.features.map((feature, index) => (
                                    <li key={index} className={`flex items-center gap-3 text-[12px] ${feature.active ? "text-[#E5E7EB]" : "text-[#9197A1]"}`}>
                                        {feature.active ? (
                        ...
                        <div key={plan.id} className="w-full max-h-fit flex flex-col gap-4 bg-[#000000] border border-[#222733] p-8 rounded-lg">
                            <h2 className="flex justify-between text-[12px] font-bold text-[#9CA3AF]">{plan.tier} <span>{plan.tierLabel}</span></h2>
                            <h3 className="text-[24px] font-bold text-white">{plan.name}</h3>
                            <p className="text-[12px] text-[#9CA3AF]">
                                {plan.description}
                                    ...
                                    <li key={index} className={`flex items-center gap-3 text-[12px] ${feature.active ? "text-[#E5E7EB]" : "text-[#9197A1]"}`}>
                                        {feature.active ? (
                                            ...
                                            <svg width="10" height="2" viewBox="0 0 10 2" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M0 1.33333V0H9.33333V1.33333H0Z" fill="#9197A1"/>
                                            </svg>
                                        )}
                                        {feature.text}
                                    </li>
